...
class NotificationControllerTest {
        ...
        void createEmailNotification_returnsNotification_whenValidRequest() throws Exception {
                ...
                when(notificationService.createNotification(eq(userId.toString()), eq(NotificationType.EMAIL),
                                eq("Test Body")))
                ...
                when(emailNotificationService.sendEmail(eq("test@example.com"), eq("Test Subject"), eq("Test Body")))
                                .thenReturn(true);
                when(notificationService.updateNotificationStatus(expectedNotification.getId(),
                                NotificationStatus.SENT))
                                .thenReturn(expectedNotification);
                when(notificationService.createEmailNotification(any(Notification.class), eq("Test Subject"),
                                eq("Test Body"), eq("test@example.com")))
        ...
        void createInAppNotification_returnsNotification_whenValidRequest() throws Exception {
                ...
                when(notificationService.createNotification(userId.toString(), NotificationType.IN_APP, "Test Body"))
                                .thenReturn(expectedNotification);
                when(notificationService.createInAppNotification(any(Notification.class), eq("Test Title"),
                                eq(InAppType.INFO), eq("Test Body")))
