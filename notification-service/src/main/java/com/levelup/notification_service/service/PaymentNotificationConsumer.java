...
public class PaymentNotificationConsumer {
        ...
        public void handlePaymentNotification(PaymentNotificationMessage message) {
                try {
                        ...
                        if (userEmail == null) {
                                log.error("Cannot send email notification - user email not found for user: {}",
                                                message.getUserId());
                                notificationService.updateNotificationStatus(notification.getId(),
                        ...
                        if (emailSent) {
                                ...
                                log.info("Payment notification email sent successfully to user: {}",
                                                message.getUserId());
                        } else {
                                notificationService.updateNotificationStatus(notification.getId(),
                                                NotificationStatus.FAILED);
        ...
        private String buildNotificationContent(PaymentNotificationMessage message) {
                return switch (message.getEventType()) {
                        ...
                        case "SUBSCRIPTION_FAILED" -> String.format("Subscription payment failed for %s %s",
                                        message.getAmount(), message.getCurrency());
                        case "REFUND_SUCCESS" -> String.format("Refund processed successfully for %s %s",
                                        message.getAmount(), message.getCurrency());
                        case "REFUND_FAILED" -> String.format("Refund processing failed for %s %s",
                                        message.getAmount(), message.getCurrency());
                        case "CANCELLATION_SUCCESS" -> String.format("Subscription cancelled successfully for %s",
                                        message.getSubscriptionName());
                        case "CANCELLATION_FAILED" -> String.format("Subscription cancellation failed for %s",
                                        message.getSubscriptionName());
        ...
        private String buildEmailSubject(String eventType) {
                return switch (eventType) {
                        ...
                        case "REFUND_SUCCESS" -> "Refund Processed - Subscription Refunded";
                        case "REFUND_FAILED" -> "Refund Failed - Unable to Process";
                        case "CANCELLATION_SUCCESS" -> "Subscription Cancelled - Confirmation";
                        case "CANCELLATION_FAILED" -> "Subscription Cancellation Failed";
        ...
        private String buildEmailBody(PaymentNotificationMessage message) {
                return switch (message.getEventType()) {
                        case "PURCHASE_SUCCESS" -> String.format("""
                                        ...
                                        message.getCourseNames() != null ? String.join(", ", message.getCourseNames())
                                                        : "N/A");
                        ...
                        case "SUBSCRIPTION_SUCCESS" ->
                                String.format("""
                                                ...
                                                message.getAmount(),
                                                message.getCurrency().toUpperCase(),
                                                message.getSubscriptionName() != null ? message.getSubscriptionName()
                                                                : "Premium Subscription",
                                                message.getInvoicePdfUrl() != null
                                                                ? "You can download your invoice from: "
                                                                                + message.getInvoicePdfUrl()
                                                                : "");
                        ...
                        case "REFUND_FAILED" -> String.format("""
                                        Dear Student,

                                        We were unable to process your refund request for %s %s.

                                        Subscription: %s

                                        Please contact our support team for assistance with your refund request.
                        ...
                        case "CANCELLATION_FAILED" -> String.format("""
                                        Dear Student,
                                        ...
                                        Your subscription remains active and billing will continue as scheduled.
